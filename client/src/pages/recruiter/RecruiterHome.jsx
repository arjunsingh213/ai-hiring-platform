...
const RecruiterHome = () => {
    ...
    const fetchStats = async () => {
        try {
            ...
            try {
                const allApplicantsResponse = await api.get(`/jobs/recruiter/${userId}/all-applicants`);
                const applicants = Array.isArray(allApplicantsResponse) ? allApplicantsResponse :
                    Array.isArray(allApplicantsResponse?.data) ? allApplicantsResponse.data : [];
                interviewsScheduled = applicants.filter(a => a.interview?.status === 'completed').length;
            } catch (err) {
                console.log('Could not fetch applicants');
            }
        ...
        <div className="home-feed">
            ...
            <div className="stats-row" style={{
                ...
                <div
                    ...
                    style={{ cursor: 'pointer', padding: 'var(--spacing-md)', textAlign: 'center' }}
                >
                    <h3 style={{ margin: 0, color: 'var(--primary)', fontSize: '1.5rem' }}>{stats.activeJobs}</h3>
                    <p className="text-muted" style={{ margin: 0, fontSize: '0.875rem' }}>Active Jobs</p>
                </div>
                <div
                    className="mini-stat card"
                    ...
                    style={{ cursor: 'pointer', padding: 'var(--spacing-md)', textAlign: 'center' }}
                >
                    <h3 style={{ margin: 0, color: 'var(--accent)', fontSize: '1.5rem' }}>{stats.totalApplicants}</h3>
                    <p className="text-muted" style={{ margin: 0, fontSize: '0.875rem' }}>Applicants</p>
                </div>
                <div
                    className="mini-stat card"
                    onClick={() => navigate('/recruiter/applications')}
                    style={{ cursor: 'pointer', padding: 'var(--spacing-md)', textAlign: 'center' }}
                >
                    <h3 style={{ margin: 0, color: 'var(--success)', fontSize: '1.5rem' }}>{stats.interviewsScheduled}</h3>
            ...
            <div className="post-creator card">
                ...
                <div className="creator-actions">
                    ...
                    <button className="action-btn" onClick={() => navigate('/recruiter/post-job')}>
                        <svg width="20" height="20" viewBox="0 0 24 24" fill="none">
